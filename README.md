...
- **マークダウンビューア/エディタ** - .md ファイルのブラウズ・閲覧・編集（GFM・シンタックスハイライト対応）
- **画像アップロード** - ターミナルへの画像ペースト対応
...
| `MD_MAX_FILE_SIZE` | `1048576` (1MB) | マークダウンファイルの最大サイズ |
| `UPLOAD_MAX_SIZE` | `10485760` (10MB) | 画像アップロードの最大サイズ |
...
| 画像アップロード | カメラ撮影やライブラリからの画像挿入 |
| マークダウンビューア | ツールバーから .md ファイルの閲覧・編集 |

### マークダウンビューア/エディタ

ツールバーの MD ボタンからマークダウンファイルを操作できます。

- ホームディレクトリ配下の `.md` ファイルをブラウズ
- marked.js による GFM（GitHub Flavored Markdown）レンダリング
- highlight.js によるコードブロックのシンタックスハイライト
- 閲覧/編集モードの切り替え、既存ファイルの上書き保存
- 隠しファイル/ディレクトリの表示切り替え
...
| GET | `/api/files/browse` | ディレクトリ一覧取得（.md ファイルとディレクトリ） |
| GET | `/api/files/read` | .md ファイルの内容取得 |
| PUT | `/api/files/write` | .md ファイルの上書き保存 |
